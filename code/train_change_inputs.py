...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, inputs, attention_mask, token_type_ids, targets=[]):
        self.inputs = inputs
        self.attention_mask = attention_mask
        self.token_type_ids = token_type_ids
        self.targets = targets
        print(len(self.inputs), len(self.attention_mask), len(self.token_type_ids), len(self.targets))
...
class Dataloader(pl.LightningDataModule):
    ...
    def tokenizing(self, dataframe):
        ...
        return data, attention_mask, token_type_ids
    ...
    def preprocessing(self, data):
        ...
        try:
            ...
            targets = []
        # 텍스트 데이터를 전처리합니다.
        inputs, attention_mask, token_type_ids = self.tokenizing(data)

        return inputs, attention_mask, token_type_ids, targets
    ...
    def setup(self, stage='fit'):
        if stage == 'fit':
            ...
            if self.k_fold:
                ...
                train_inputs, train_attention_mask, train_token_type_ids, train_targets = self.preprocessing(train_data)
                val_inputs, val_attention_mask, val_token_type_ids, val_targets = self.preprocessing(val_data)

                self.train_dataset = Dataset(train_inputs, train_attention_mask, train_token_type_ids, train_targets)
                self.val_dataset = Dataset(val_inputs, val_attention_mask, val_token_type_ids, val_targets)
    ...
    def collate_fn(self, batch):
        ...
        maxl = 0
        for _input, _att_mask, _type_ids, _label in batch:
            if pad not in _input.tolist():
                maxl = 256
            elif _input.tolist().index(pad) > maxl: 
                maxl = _input.tolist().index(pad)
            
            data_list['input_ids'].append(_input)
            data_list['attention_mask'].append(_att_mask)
            data_list['token_type_ids'].append(_type_ids)
        ...
        #print(data_list['input_ids'])
        
        return torch.stack(data_list['input_ids'], dim=0).long(), torch.stack(data_list['attention_mask'], dim=0).long(), torch.stack(data_list['token_type_ids'], dim=0).long(), torch.tensor(label_list).long()
...
class Model(pl.LightningModule):
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        ...
        sep_idx = [item.tolist().index(sep) for item in input_ids]  
    ...
    def validation_step(self, batch, batch_idx):
        x, att_mask, type_ids, y = batch
        logits = self(x, att_mask, type_ids)
        loss = self.loss_func(logits, y.float())
